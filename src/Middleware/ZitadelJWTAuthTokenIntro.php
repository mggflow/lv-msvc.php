...
use function MGGFLOW\LVMSVC\Auth\Zitadel\introspect_token_via_JWT_auth;
use function MGGFLOW\LVMSVC\Auth\Zitadel\validate_intro;

class ZitadelJWTAuthTokenIntro
{
    /**
    ...
    public function handle(Request $request, Closure $next): Response
    {
        ...
        $intro = Cache::remember($cacheKey, $config->tokenIntrospectionPeriod, function () use ($token, $config) {
            return introspect_token_via_JWT_auth($token, $config, $config->introAssoc);
        });
